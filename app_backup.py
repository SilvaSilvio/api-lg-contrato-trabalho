...
class TelaContratos:
    ...
    def atualizar_area_administrativa(self):
        ...
        try:
            ...
            if hasattr(self, 'main_container') and self.main_container:
                # Garantir que o main_container seja reconstruído para refletir as mudanças
            ...
            # Atualizar a página ao final para refletir todas as mudanças
            self.page.update()
        except Exception as e:
    ...
    def confirmar_exclusao_empresa(self, e, dlg, codigo):
        ...
        try:
            ...
            if sucesso:
                ...
                # Reconstruir o main_container para garantir que a interface seja atualizada
                if hasattr(self, 'main_container') and self.main_container:
                    if self.main_container.content:
                        current_content = self.main_container.content
                        self.main_container.content = None
    ...
    def editar_operador(self, e):
        """Preenche o formulário com os dados do operador para edição."""
        # Obter dados do operador
        operador = self.operador_controller.get_operador(e.control.data)
        if not operador:
            self.page.show_snack_bar(
                ft.SnackBar(
        ...
        self.senha_operador_field.value = ""  # Não preencher a senha por segurança
        self.ativo_operador_checkbox.value = operador['ativo']
        
        # Armazenar referência do operador sendo editado
        self.operador_editando = e.control.data
    ...
    def salvar_operador(self, e):
        """Salva as alterações de um operador."""
        # Validar campos obrigatórios
        if not self.email_operador_field.value:
            snack = ft.SnackBar(content=ft.Text("Por favor, preencha o email do operador"))
            self.page.overlay.append(snack)
            snack.open = True
            self.page.update()
            return
        ...
        # Atualizar operador
        sucesso, mensagem = self.operador_controller.atualizar_operador(
            self.operador_editando,  # Email original do operador
            self.email_operador_field.value,
            self.senha_operador_field.value if self.senha_operador_field.value else None,
    ...
    def adicionar_operador(self, e):
        ...
        # Criar operador
        sucesso, mensagem = self.operador_controller.criar_operador(
            self.email_operador_field.value,
            self.senha_operador_field.value,
            self.ativo_operador_checkbox.value
        )
    ...
    def excluir_usuario(self, e, usuario):
        """Exclui um usuário."""
        ...
        dlg = ft.AlertDialog(
            ...
            actions=[
                ft.TextButton("Cancelar", on_click=lambda e: self.fechar_dialogo(e, dlg)),
                ft.TextButton("Excluir", on_click=lambda e: self.confirmar_exclusao_usuario(e, dlg, usuario["email"]))
            ]
        )
    ...
    def confirmar_exclusao_usuario(self, e, dlg, email):
        ...
        # Excluir usuário
        sucesso, mensagem = self.usuario_controller.excluir_usuario(email)
    ...
    def editar_usuario(self, e, usuario):
        """Inicia a edição de um usuário."""
        # Preencher campos com dados do usuário
        self.nome_usuario_field.value = usuario["nome"]
        self.email_usuario_field.value = usuario["email"]
        self.senha_usuario_field.value = ""  # Não preencher senha por segurança
        self.admin_usuario_checkbox.value = usuario["admin"]
        
        # Armazenar email original para atualização
        self.email_usuario_original = usuario["email"]
